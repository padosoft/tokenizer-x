...
{
    public function gpt_utf8_encode(string $str): string
    {
        $str .= $str;
        $len = \strlen($str);
    ...
    public function gpt_encode($text)
    {
        $bpe_tokens = [];
        if (empty($text)) {
            return $bpe_tokens;
        }
        $raw_chars = file_get_contents(dirname(__FILE__).'/OpenAI/characters.json');
        ...
        $rencoder = file_get_contents(dirname(__FILE__).'/OpenAI/encoder.json');
        ...
        $bpe_file = file_get_contents(dirname(__FILE__).'/OpenAI/vocab.bpe');
        if (empty($bpe_file)) {
    ...
    public function gpt_dictZip($x, $y)
    {
        $result = [];
        $cnt = 0;
        foreach ($x as $i) {
    ...
    public function gpt_get_pairs($word)
    {
        $pairs = [];
        $prev_char = $word[0];
    ...
    public function gpt_split($str, $len = 1)
    {
        $arr = [];
        if (function_exists('mb_strlen')) {
    ...
    public function gpt_bpe($token, $bpe_ranks, &$cache)
    {
    ...
    public function gpt_indexOf($arrax, $searchElement, $fromIndex)
    {
        $index = 0;
